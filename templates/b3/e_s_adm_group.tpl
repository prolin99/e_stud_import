      ...
      <div class="row">
      	...
      	<div class="col-md-10" data_ref="group_<{$g_key}>" id="group_<{$g_key}>">
      		<{foreach  key=u_key item=user    from= $data.group_users_list[$g_key] }>
      		  <span class="col-md-2" data_ref="tea_<{$u_key}>_<{$user.name}>_group_<{$g_key}>"><label class="label label-success"><{$user.name}><span class="fa fa-trash icon-white del" title="移除"></span></label></span>
      		<{/foreach}>
      	</div>
      </div>
      <{/foreach}>
...
function set_group(user_data ,group_box) {
	...
	var user = '<span class="col-md-2" data_ref="'+ user_data + '_' +group_box +'"><label class="label label-success">'+ splits[2]+ '<span class="fa fa-trash icon-white del" title="移除"></span></label></span>' ;
