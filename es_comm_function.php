...
while($row=$xoopsDB->fetchArray($result)){
	$classname_set = $row['message'] ;
}
if ( $classname_set) 
	$ES_ClassName  = preg_split('/[,\s]/' , $classname_set ) ;


function es_class_name_c($class_id ) {
	global $ES_ClassName ;
	$grade= array (1=>'一' ,2=>'二' , 3=>'三' , 4=>'四' , 5=>'五' , 6=>'六' , 7=>'七' , 8=>'八' , 9=>'九'   ) ;

	if ($ES_ClassName) {
		$class_order = substr($class_id ,1,2)-1 ;
...
//取得班級列表
	...
	while($data_row=$xoopsDB->fetchArray($result)){
 	 	$class_list[$data_row['class_id']]= es_class_name_c($data_row['class_id'] );
	}	
	return $class_list ; 
}
